...
import { GridColliderObject } from "./interfaces";

const cellSize10 = 10;

describe("GridCollider ordinate test", () => {
  const testCases = [
  ...
  testCases.forEach(({ xOrY, cellSize, expected }) => {
    it(`ordinate - return ${expected} for input ${xOrY} with cell size ${cellSize}`, () => {
      const collider = new GridCollider({
        worldWidth: 100,
        worldHeight: 100,
        cellSize,
      });
...
describe("GridCollider getCurrentObjectCells", () => {
  let collider: GridCollider;
  beforeEach(() => {
    const worldWidth = 100;
    const worldHeight = 100;
    const cellSize = 10;
    collider = new GridCollider({ worldWidth, worldHeight, cellSize });
  });

  it("small object", () => {
    const testObject: GridColliderObject = {
      id: "test",
      left: 2,
      top: 2,
      width: 5,
    ...
    const placedCells = collider.getCurrentObjectCells(testObject);
    ...
    placedCells.forEach((cell) => {
      expect(cell.data).toHaveProperty("test");
    });
  });
  ...
  it("small object", () => {
    ...
    const worldHeight = 100;
    const cellSize = 10;
    const collider = new GridCollider({ worldWidth, worldHeight, cellSize });
...
describe("GridCollider getFirstCollisionObject", () => {
  let collider: GridCollider;

  beforeEach(() => {
    collider = new GridCollider({
      worldWidth: 100,
      worldHeight: 100,
      cellSize: cellSize10,
    });
