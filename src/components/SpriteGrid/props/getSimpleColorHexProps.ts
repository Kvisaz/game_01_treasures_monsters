...
import { ISpriteGridColorCellData, SpriteGridColorHexCell } from "../cells";
...
export function getSimpleColorHexProps(scene: Phaser.Scene): ISpriteGridProps<ISpriteGridColorCellData> {
  const cellSize = 64;
  ...
  return {
    ...
    gridStrategy: new HexStrategy({ cellSize }),
    pathBuilder: new SimplePathBuilder({ scene, cellSize })
  };
}

function getRandomColor() {
  return getRandomArrayElementWithFilter(["#b47c00",
    "#4d725c", "#fff6d6", "#8c8cb6", "#dedede",
    "#af0000"]);
}
